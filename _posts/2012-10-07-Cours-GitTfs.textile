...
Parmis les contrôleurs de code source, "TFS est celui le plus détesté par les développeurs :( et Git celui le plus apprécié":http://www.infoq.com/news/2010/03/Review-Version-Control-Systems .
...
Convient pour un besoin ponctuel ou qu'on n'a pas besoin de l'historique...
...
* on peut refiler le dépot cloné par copie (ou par clonage git!) à quelqu'un
...
|_. Name|_. Cmd.|_. Arguments|
|Tfs(Branche)> Fetch|@sh@|<code>-c "git tfs fetch -i $(echo {sRemoteBranch} | sed 's/tfs\///')"</code> |
|Tfs(Branche)> Fetch and rebase|@sh@|pre. -c "git tfsrebasebranch $(echo {sRemoteBranch} | sed 's/tfs\///')" |
|Tfs(Branche)> Commit|@sh@|bc.. -c "git tfscommitbranch $(echo {sRemoteBranch} | sed 's/tfs\///')"
 |
...
* Visual Studio est plus rapide car il n'a pas à contacter le serveur TFS pour extraire les fichiers en modifications
