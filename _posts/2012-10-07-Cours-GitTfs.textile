...
Le controleur de code source TFS(Team Foundation Server) est bien souvant utilisé en entreprise.
...
Pour le moment, la gestion des branches par git-tfs que j'ai développée n'étant pas dans encore dans la version officielle 0.15 de git-tfs (pull request en attente de validation sur GitHub), on utilisera plutôt une "version personnelle de git-tfs":https://github.com/downloads/pmiossec/git-tfs/git-tfs-with-branches.zip 
...
Note : On pourra tester toutes les commandes git-tfs indiquées en utilisant le projet "valtechgittfs" disponible sur "Codeplex":http://valtechgittfs.codeplex.com/
Pour cela, on pourra utiliser le compte utilisateur 'vtccds' (mot de passe:vtccds)
