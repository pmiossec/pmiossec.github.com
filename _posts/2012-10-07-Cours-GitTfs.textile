...
{border:1px solid black}. |Tfs(Tronc)> Fetch|git|tfs fetch|
{border:1px solid black}. |Tfs(Tronc)> Fetch and Rebase|git|tfsrebase|
{border:1px solid black}. |Tfs(Tronc)> Commit|git|tfscommit|

Pour info mais non utile:

{border:1px solid black}. |Tfs(Tronc)> Commit|git|tfs rcheckin|
{border:1px solid black}. |Tfs(Tronc)> Commit Visuel|git|tfs ct|
...
* Le 3ème paramètre, correspondant à l'identifiant TFS du dernier commit commun entre le tronc et la branche. TFS n'est pas très utile pour le trouver :(
Pour cela, depuis l'explorateur de code source de TFS, faire "Afficher l'historique" sur la branche, le n° de la dernière ligne étant le n° du 1er commit dans la branche.
Il faut trouver le commit parent. A vous de jouer :( Bien souvent, il suffit de prendre le commit précédent (donc soustraire 1) mais c'est à vérifier absolument!!
...
{border:1px solid black}. |Tfs(Branche)> Fetch|sh|-c "git tfs fetch -i $(echo {sRemoteBranch} &#124; sed 's/tfs\///')"|
{border:1px solid black}. |Tfs(Branche)> Fetch and rebase|sh|-c "git tfsrebasebranch $(echo {sRemoteBranch} &#124; sed 's/tfs\///')"|
{border:1px solid black}. |Tfs(Branche)> Commit|sh|-c "git tfscommitbranch $(echo {sRemoteBranch} &#124; sed 's/tfs\///')"|
...
{border:1px solid black}. |Tfs(Branche)> Commit|sh|-c "git tfs rcheckin -i $(echo {sRemoteBranch} &#124; sed 's/tfs\///')"|
