...
Nous allons donc rajouter un alias pour nous faciliter grandement la vie lorsqu'une commande git ou git-tfs nécessite un espace de travail sans modification.
...
Pour synchroniser ses modifications avec la dernière version de TFS (en utilisant l'alias 's' défini ci-dessus) :
...
|Tfs.Fetch|@git@|@tfs fetch@|
|Tfs.Rebase|@git@|@s tfs pull --rebase@|
|Tfs.Commit|@git@|@s tfs rcheckin --quick --autorebase@|
...
A l'identique des commandes utilisées pour travailler sur la branche principale, on utilisera les commandes suivantes ( avec l'option -i qui permet d'indiquer sur quelle branche tfs on va travailler)...
...
bc. git s tfs pull --rebase -i branche1

Commiter sur la branche sur TFS:

bc. git s tfs rcheckin --quick -a -i branche1

h3. 5.7.Scripts GitExtension (suite):

Note (pour information) : les commandes que l'on doit indiquer à GitExtension sont un peu plus complexe que celle fournies lors du travail sur la branche principale car il faut extraire le nom de la branche requis par la commande git-tfs d'une variable {sRemoteBranch} fournie par GitExtension et qu'il n'y a pas de manière simple de le faire :(

Menu "Settings"->"Settings..."->Onglet "Scripts" et ajoutez un ligne où on remplira les 3 champs requis :

|_. Name|_. Cmd.|_. Arguments|
|Tfs.Fetch.Branche|@sh@|<code>-c "git tfs fetch -i $(echo {sRemoteBranch} </code>&#124;<code> sed 's/tfs\///')"</code> |
|Tfs.Rebase.Branche|@sh@|<code>-c "git s tfs pull --rebase -i $(echo {sRemoteBranch} </code>&#124;<code> sed 's/tfs\///')"</code> |
|Tfs.Commit.Branche|@sh@|<code>-c "git s tfs rcheckin --quick --autorebase -i $(echo {sRemoteBranch} </code>&#124;<code> sed 's/tfs\///')"</code> |
