...
h2. Utilisation de Git en environnement hostile (... avec un serveur TFS)

h3. Cloner la branche principale du dépôt tfs
...
h3. Synchronisation avec le dépot tfs

h4. Récupération des commits du tronc

@git tfs fetch@

h4. Commiter sur le dépot tfs

@git tfs ct@

! Toujours faire une fetch & rebase (ou merge) avant de commiter (sinon risque d'écrasement de fichier communs modifiés)

h3. Gestion d'une branche

h4. Récupération de la branche
...
h4. Récupération des commits d'une branche

@git tfs fetch -i maBrancheTfs@

h4. Commiter sur une branche
